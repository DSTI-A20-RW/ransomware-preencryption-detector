...
def main_malware():
    ...
    preselected_features_path = os.path.join(os.getcwd(), 'selected_features/malware')
    ...
    for target, elements in processing_elements.items():
        ...
        for file in json_files_list:
            ...
            if elements[1] is not None:
                args = (file, elements[1])
            try:
                df = elements[0](*args)
                if df is not None:
                    dfs.append(df)
            except:
                print(f'Processing element failed!')

        #concatenate the resulting one line dataframes in one dataframe and append
        try:
            result_df = pd.concat(dfs, axis=0, ignore_index=False).replace(np.nan, 0).astype(np.int32)
        except:
    ...
    #save result
    result_path = os.getcwd() + os.sep + 'dataset/result_malware.pkl.gz'
    df_base.to_pickle(result_path, compression='gzip')
    #result_path = os.getcwd() + os.sep + 'dataset/result_malware.csv'
    #print(df_base.shape)
    #df_base.to_csv(result_path)
...
def main_ransomware():
    ...
    #define the path to predefined columns to use for initial feature selection
    preselected_features_path = os.path.join(os.getcwd(), 'selected_features/ransomware')
    ...
    #save result
    result_path = os.getcwd() + os.sep + 'dataset/result_ransomware.pkl.gz'
    df_base.to_pickle(result_path, compression='gzip')
    #result_path = os.getcwd() + os.sep + 'dataset/result_ransomware.csv'
    #print(df_base.shape)
    #df_base.to_csv(result_path)
